...
def PrimsCity(input = sys.stdin):
	...
	while len(TravelHeap) !=0:
		...
		if type(poppedCity[1]) is int:
			setA.append(poppedCity)
			for cities in setA:
				for others in Neghbrs[cities[2]]:
					TravelHeap.update_if_better(others,int(Neghbrs[cities[2]][others]),cities[2])
					
	#prints the answer
	print sum((x[1] for x in setA))
	for x in setA:
		TravelHeap.Display(x)
